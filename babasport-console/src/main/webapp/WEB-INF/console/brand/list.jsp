...
品牌名称: <input type="text" name="name" value="${brandQuery.name}"/>
	<select name="isDisplay">
		<option value="1" <c:if test="${brandQuery.isDisplay==1}">selected</c:if>>是</option>
		<option value="0" <c:if test="${brandQuery.isDisplay==0}">selected</c:if>>否</option>
	</select>
	<input type="submit" class="query" value="查询"/>
</form>
	<form id="jvform" action="/brand/delete.do" method="post">
...
<table cellspacing="1" cellpadding="0" border="0" width="100%" class="pn-ltable">
	...
	<c:forEach items="${pagination.list}" var="brand" >
		<tr bgcolor="#ffffff" onmouseout="this.bgColor='#ffffff'" onmouseover="this.bgColor='#eeeeee'">
			...
			<td align="center">
				<a class="pn-opt" href="/brand/edit.do?brandId=${brand.id}">修改</a> | <a class="pn-opt" onclick="if(!confirm('您确定删除吗？')) {return false;}" href="#">删除</a>
			</td>
		</tr>
	</c:forEach>
	</tbody>
...
<div class="page pb15">
	<span class="r inb_a page_b">
		<c:forEach items="${pagination.pageView}" var="pagination">
			${pagination}
		</c:forEach>
	</span>
</div>
